 ...
 * **`total` is capped by `candidate_limit`** (default 5000). When more documents match, only the 5000 that contain the most of the user's original query words are scored and counted. Set `'candidate_limit' => 0` when an exact total matters more than latency; it then scores every matching document, at roughly 600 bytes and 20 microseconds each (127K matches on the Wikipedia corpus: about 80 MB and 5 seconds).
 ...
 * **Memory is proportional to matched documents, not posting rows.** The first pass streams postings into one coverage bitmask per document (about 40 bytes each) and only the pruned survivors have their posting rows materialized. Required and phrase words are resolved by rarest-first intersection, so `"the united states of america"` looks up `the` for the 13K `america` documents instead of loading all 270K of its rows. Excluded words are only fetched for the survivors. Past the prune, each scored candidate costs about 600 bytes: its posting cells (stored term-major, so there is no per-document array), its field lengths (field-major, same reason), one float for its score, and one flag per field it appears in. The per-field score breakdown and matched-term lists that `hits` carry are computed only for the page being returned, not for every candidate. On the Wikipedia corpus every query shape tested stays under 65 MB at the default `candidate_limit`, most under 25 MB, and `'candidate_limit' => 0` on `the united states of america` (127K matched documents scored) peaks near 80 MB. The earlier design kept a row per posting in PHP arrays, about 450 bytes each, which put stopword phrases and exclusions at 300 MB and ordinary three-word queries near 100 MB, and later kept a score breakdown, a match list and a length hash per candidate, which put that exhaustive query at 300 MB; the test suite carries memory budgets for both shapes so they stay fixed.
